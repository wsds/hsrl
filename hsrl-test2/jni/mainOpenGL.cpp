...
namespace hsrl {
	...
	// mModelMatrixģ�;���mViewMatrix��ͼ����mProjectionMatrix͸�Ӿ���
	...
	void setupProjectionView(int width, int height){
		...
		mProjectionMatrix = mMatrix4Helper->OrthoM(-1.0f, 1.0f, -1.0f / ratio, 1.0f / ratio, 1.0f, 11.0f);// ��ͶӰ//near at z=1//far at z=11
	...
	void renderFrame() {
		...
		if (isInitailized == false) {
			...
			//glUniformMatrix4fv(mProjectionViewMatrixHandle, 1, false, value);
			//glUniformMatrix4fv(mProjectionViewMatrixHandle, 1, false, mProjectionViewMatrix->Ptr());//�ظ���
			GLfloat* value = mMatrix4Helper->getDataCopy(mProjectionViewMatrix);
			glUniformMatrix4fv(mProjectionViewMatrixHandle, 1, false, value);
			isInitailized = true;
		}
